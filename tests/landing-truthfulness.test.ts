...
describe("landing-page truthfulness", () => {
  it("provides a clear product title, description, canonical URL, and social metadata", () => {
    expect(landing).toContain("<title>MCP Hub — Wrangle Every MCP Server</title>");
  ...
  it("keeps primary calls to action on owned access paths rather than an unowned app domain", () => {
    ...
    expect(landing).toContain('href="./docs.html"');
