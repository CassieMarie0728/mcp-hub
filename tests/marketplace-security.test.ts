...
function getRouteHandler(path: string): any {
  const route = marketplaceRouter.stack.find((layer: any) => layer.route?.path === path);
  if (!route || !route.route || !route.route.stack || route.route.stack.length === 0) {
    throw new Error(`Route handler for path ${path} not found`);
  }
...
describe('Marketplace Router Security', () => {
  ...
  it('should return 403/401 when authentication fails for POST /macros/:id/download', async () => {
    ...
    const handler = getRouteHandler('/macros/:id/download');
  ...
  it('should return 403/401 when authentication fails for POST /macros/:id/reviews', async () => {
    ...
    const handler = getRouteHandler('/macros/:id/reviews');
