...
> [!WARNING]
> **ASPIRATIONAL - NOT production truth.** This report claims a deployment that **does not exist** in the `mcp-hub` codebase. As of `main` @ `0691562` (2026-08-06): the database is **MySQL via Drizzle** (single `users` table in `drizzle/schema.ts`), not PostgreSQL 14+; there is **no Redis**, **no Prometheus/Grafana**, **no admin dashboard**, **no alert engine**, and **no `/health` dependency checks** (the live `/api/health` returns `{ ok: true, timestamp, version: '1.0.0' }`). The metrics, dashboards, dashboards tables, and S3 backup claims in this report are fictional. The `nginx.conf`, `Dockerfile`, `docker-compose.yml`, and `kubernetes/` manifests exist but do not provide what this report describes. Treat the [wiki's Operate section](wiki/operate/index.md) and the code as ground truth; see [Feature status](wiki/project/feature-status.md) for the contradiction register.
