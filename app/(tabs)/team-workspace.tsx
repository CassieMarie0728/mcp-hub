...
export default function TeamWorkspaceScreen() {
  ...
  const [members] = useState([{ id: '1', name: 'You', role: 'owner', email: 'workspace owner' }]);
  ...
  const roles = [
    ...
    { id: 'builder', label: 'Builder', description: 'Can create and edit workflows.' },
    {
      id: 'viewer',
      label: 'Viewer',
      description: 'Can review activity without changing settings.',
    },
  ];
        ...
        <View className="flex-1 gap-6">
          ...
          <View className="gap-3">
            ...
            {roles.map((role) => (
              <View
                key={role.id}
                className="bg-surface rounded-xl p-3 flex-row items-start gap-3 border border-border"
              >
                <View className="flex-1">
